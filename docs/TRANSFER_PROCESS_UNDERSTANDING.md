...
2. **Confirmed decisions:**
   ...
   - **The sale is recorded in the main ERP portal** as a Raw-Jute Sales Invoice (`invoice_type=5`); **"sale completed" = that invoice reaching Approved (`status_id=3`)**. juteTransfer only reacts/backfills — it gets no sale-entry UI for marked stock. **(As originally decided here. See Amendment 2026-08-03 below: this described the ERP-side ONWARD sale/consumption at the target — the transfer itself now additionally books its own inter-company seller invoice at move time, which is separate from and prior to this ERP-side sale.)**
...
**Amendment 2026-08-03:** §3 and §6.2 above described type-2 moves as having no sales invoice — that's superseded. `save_marked_batch` now also books one seller Raw-Jute `sales_invoice` (`invoice_type=5`) per child MR, from the source branch to the target company's party (auto-created if missing), and stamps the child MR with that invoice's no/date/amount; `sales_invoice_jute.mr_id` links to the child MR id for deletion (different semantics from type 1's hop linkage), and `delete_marked_move` cascades the invoice on undo. This invoice books the *inter-company* sale at transfer time — it is distinct from, and prior to, the *onward* sale/consumption at the target company, which still happens in the ERP portal exactly as described in §3.2 (Raw-Jute invoice reaching Approved, or now, balance-based via §6.3).
