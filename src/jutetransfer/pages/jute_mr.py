...
def _render_transfer_editor(mr_id: int, row: pd.Series, steps: list,
    ...
    if num_saved > 0 and len(steps) <= num_saved:
        ...
        for sc in saved_chain:
            ...
            saved_step["total_amount"] = float(sc.get("total_amount") or 0)
                ...
                if new_co and new_co in co_branch_mapping:
                    ...
                    if new_wh and warehouses and new_wh in warehouses:
                        if warehouses[new_wh] != current_wh_id:
                            ...
                            changed = True
                    if not warehouses:
                        st.caption("No warehouses found")
    ...
    if unsaved_with_company:
        ...
        with col_actions[0]:
            if st.button(f"Save Step {step_idx + 1}", key=f"save_step_{filter_key}_{mr_id}",
                         type="primary"):
                with st.spinner("Saving transfer step..."):
                    try:
                        ...
                        if co_label not in co_branch_mapping:
                            ...
                            # Compute cumulative rate multiplier
                            cumulative_multiplier = 1.0
                            for j in range(1, step_idx + 1):
                                pct = float(steps[j].get("pct_rate_increase", 0) or 0)
                                cumulative_multiplier *= (1 + pct / 100)
